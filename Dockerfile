...
# and `pip install creditFraud` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "creditFraud", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n creditFraud -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "creditFraud", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "creditFraud", "YOUR_CONSOLE_SCRIPT"]
